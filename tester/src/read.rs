...
pub(super) fn read_harness(test262: &Path) -> io::Result<Harness> {
    let mut includes = FxHashMap::default();

    for entry in fs::read_dir(test262.join("harness"))? {
        let entry = entry?;
    ...
    let assert = fs::read_to_string(test262.join("harness/assert.js"))?.into_boxed_str();
    let sta = fs::read_to_string(test262.join("harness/sta.js"))?.into_boxed_str();
...
pub(super) fn read_global_suite(test262: &Path) -> io::Result<TestSuite> {
    let path = test262.join("test");
