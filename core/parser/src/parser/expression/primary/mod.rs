    ...
    fn parse(self, cursor: &mut Cursor<R>, interner: &mut Interner) -> ParseResult<Self::Output> {
        ...
        match tok.kind() {
            ...
            TokenKind::Punctuator(Punctuator::OpenParen) => {
                // Tracking depth with a RAII guard to ensure it stays in sync.
                let guard = cursor.enter_parenthesized();

                let next_is_also_paren = cursor.peek(1, interner)?.map(Token::kind)
                    == Some(&TokenKind::Punctuator(Punctuator::OpenParen));

                if next_is_also_paren && guard.depth() >= Cursor::<R>::FAST_PATH_PAREN_DEPTH {
