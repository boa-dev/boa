...
impl<K: Trace + ?Sized, V: Trace> Ephemeron<K, V> {
    ...
    #[must_use]
    pub fn new(_mc: &crate::MutationContext<'_, '_>, key: &Gc<'_, K>, value: V) -> Self {
    ...
    #[inline]
    #[must_use]
    pub fn key<'gc>(&self, _mc: &crate::MutationContext<'gc, '_>) -> Option<Gc<'gc, K>> {
    ...
    pub fn value<'gc>(
        &self,
        _mc: &crate::MutationContext<'gc, '_>,
    ) -> Option<EphemeronValueRef<'gc, K, V>> {
        let key = self.key(&unsafe { crate::MutationContext::dummy() })?;
