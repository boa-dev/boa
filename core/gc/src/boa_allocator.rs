...
pub(crate) type ErasedWeakMapBoxPointer = NonNull<dyn ErasedWeakMapBox>;

thread_local!(pub(crate) static GC_DROPPING: Cell<bool> = const { Cell::new(false) });
thread_local!(pub(crate) static BOA_GC: RefCell<BoaGc> = RefCell::new( BoaGc {
...
pub(crate) struct GcConfig {
    ...
    pub(crate) used_space_percentage: usize,
}
...
pub(crate) struct GcRuntimeData {
    pub(crate) collections: usize,
    pub(crate) bytes_allocated: usize,
}

#[derive(Debug)]
pub(crate) struct BoaGc {
    pub(crate) config: GcConfig,
    pub(crate) runtime: GcRuntimeData,
    pub(crate) strongs: Vec<GcErasedPointer>,
    pub(crate) weaks: Vec<EphemeronPointer>,
    pub(crate) weak_maps: Vec<ErasedWeakMapBoxPointer>,
}
...
impl Allocator {
    ...
    pub(crate) fn alloc_gc<T: Trace>(value: GcBox<T>) -> NonNull<GcBox<T>> {
    ...
    pub(crate) fn alloc_ephemeron<K: Trace + ?Sized, V: Trace>(
