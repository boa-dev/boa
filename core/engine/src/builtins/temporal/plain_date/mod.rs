...
pub(crate) fn to_partial_date_record(
    ...
    let month_code = partial_object
        ...
        .map_or(None, |v| {
            let v = match v.to_primitive(context, crate::value::PreferredType::String) {
                Ok(val) => val,
                Err(e) => return Some(Err(e)),
            };

            let JsValue::String(month_code) = v else {
    ...
    let era = partial_object
        ...
        .map_or(None, |v| {
            ...
            let JsValue::String(era) = v else {
                return Some(Err(JsError::from(
                    JsNativeError::typ()
                        .with_message("The monthCode field value must be a string."),
                )));
            };
