...
use temporal_rs::{
    host::{HostClock, HostHooks, HostTimeZone}, now::Now as InnerNow, provider::TimeZoneProvider, unix_time::EpochNanoseconds, TemporalError, TemporalResult, TimeZone
};
...
impl HostClock for &Context {
    fn get_host_epoch_nanoseconds(&self) -> TemporalResult<EpochNanoseconds> {
        Ok(EpochNanoseconds::from(self.clock().now().nanos_since_epoch() as i128))
    }
}
