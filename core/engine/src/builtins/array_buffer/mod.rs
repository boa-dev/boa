...
pub(crate) fn assert_external_memory_alignment<T>(data: &[T]) {
    assert!(
        data.is_empty()
            || data
                .as_ptr()
...
pub(crate) enum BufferData {
    ...
    ///
    /// The exclusive `&'static mut` reference is what makes non-atomic access to
    /// the region sound: the embedder gave up its own access to the region when
    /// creating the buffer, so the engine can access it with normal loads, stores
    /// and copies, exactly like Boa-owned memory. See [`ArrayBuffer::from_external_ptr`]
    /// for the contract required of embedders that keep accessing the region.
...
impl Clone for BufferData {
    fn clone(&self) -> Self {
        match self {
            ...
            Self::External(data) => Self::Owned(AlignedVec::from_iter(0, data.to_vec())),
        }
    }
}
...
impl BufferData {
    fn slice_ref(&self) -> SliceRef<'_> {
        match self {
            ...
            Self::External(data) => SliceRef::Slice(data),
        }
    }
    ...
    fn slice_ref_mut(&mut self) -> SliceRefMut<'_> {
        match self {
            ...
            Self::External(data) => SliceRefMut::Slice(data),
        }
    }
...
impl ArrayBuffer {
    ...
    /// allocates, grows nor frees the region, and accesses it with normal (non-atomic)
    /// loads, stores and copies, exactly like Boa-owned memory.
    ///
    /// The exclusive `&'static mut` reference is what makes this sound: by passing it,
    /// the embedder gives up its own access to the region for as long as the buffer (or
    /// a view over it) can reach it. Embedders that need to keep accessing the region
    /// while the buffer is alive must either synchronize every access with the
    /// JavaScript code that may access the buffer, or create the buffer through the
    /// raw-pointer based [`ArrayBuffer::from_external_ptr`]; see that method for the
    /// full contract. For regions that are concurrently accessed from other threads,
    /// create a [`SharedArrayBuffer`] instead with
    /// [`SharedArrayBuffer::from_external_data`], which accesses the region with atomic
    /// operations.
    ///
    ...
    /// accesses need.
    #[must_use]
    pub fn from_external_data(data: &'static mut [u8]) -> Self {
    ...
    ///   [`SharedArrayBuffer`]; see [`SharedArrayBuffer::from_external_ptr`].
    ///
    /// # Panics
    ///
    ...
    pub unsafe fn from_external_ptr(ptr: *mut u8, len: usize) -> Self {
        ...
        // SAFETY: the caller guarantees that the region is valid for reads and writes
        // of `len` bytes for the whole lifetime of the buffer, and that it is not
        // accessed concurrently while the engine may access it.
    ...
    pub fn detach(&mut self, key: &JsValue) -> JsResult<Option<AlignedVec<u8>>> {
        ...
        Ok(self.data.take().map(|data| match data {
            ...
            BufferData::External(data) => AlignedVec::from_iter(0, data.to_vec()),
        }))
    }
