...
impl std::str::FromStr for Unit {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // TODO: Make SANCTIONED_UNITS store `JsStr`s

        // SAFETY: the value in the SANCTIONED_UNITS are ascii strings, so this is safe.
        let num = unsafe { JsStr::ascii_unchecked(num.as_bytes()) };
        ...
        let den = if den.is_empty() {
            ...
            // SAFETY: the value in the SANCTIONED_UNITS are ascii strings, so this is safe.
            unsafe { JsStr::ascii_unchecked(value.as_bytes()) }
        };
