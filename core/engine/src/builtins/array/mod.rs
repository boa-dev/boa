...
impl Array {
    ...
    fn set_length(o: &JsObject, len: usize, context: &mut Context) -> JsResult<()> {
        if o.is_array() && len < (u32::MAX - 1) as usize {
    ...
    pub(crate) fn from(
        ...
        //     ...
        //     ix. Set k to k + 1.
        for k in 0..(Number::MAX_SAFE_INTEGER as usize) {
    ...
    pub(crate) fn push(
        ...
        if len + arg_count > (Number::MAX_SAFE_INTEGER as usize) {
            return Err(JsNativeError::typ()
                .with_message(
    ...
    pub(crate) fn flat(
        ...
        if let Some(depth) = args.first() {
            ...
            match depth.to_integer_or_infinity(context)? {
                IntegerOrInfinity::Integer(value) if value >= 0 => depth_num = value as usize,
                IntegerOrInfinity::PositiveInfinity => depth_num = usize::MAX,
                _ => depth_num = 0,
            }
        };
        ...
        while source_index < source_len {
            ...
            if let Some(mut element) = source.try_get(p, context)? {
                ...
                // vi. Else
                    ...
                    if target_index >= Number::MAX_SAFE_INTEGER as usize {
                        return Err(JsNativeError::typ()
    ...
    pub(crate) fn splice(
        ...
        if len + item_count - actual_delete_count > Number::MAX_SAFE_INTEGER as usize {
            return Err(JsNativeError::typ()
    ...
    fn to_spliced(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
        ...
        if new_len > Number::MAX_SAFE_INTEGER as usize {
            return Err(JsNativeError::typ()
                .with_message("Target splice exceeded max safe integer value")
                .into());
        }
    ...
    pub(crate) fn with(
        ...
        let relative_index = relative_index as isize;
