...
impl Array {
    ...
    pub(crate) fn from_async(
        ...
        use crate::native_function::{ContinuationState, NativeContinuation};
        ...
        fn from_iterable(
            ...
            state_machine: &Cell<Option<IterableStateMachine>>,
            context: &mut Context,
        ) -> JsResult<ContinuationState> {
            let Some(mut sm) = state_machine.take() else {
                return Ok(ContinuationState::Done);
            };

            loop {
                match sm {
                    IterableStateMachine::LoopStart {
                        a,
                        k,
                        iterator_record,
                    } => {
                        if k >= 2u64.pow(53) - 1 {
                            sm = IterableStateMachine::AsyncIteratorCloseStart {
                                err: JsNativeError::typ()
                                    .with_message(
                                        "Array.fromAsync: \
                            ...
                            state_machine.set(Some(IterableStateMachine::LoopContinue {
                                a,
                                k,
                                iterator_record,
                            }));
                            return Ok(ContinuationState::Yielded(next_result));
                        }
                    }
                    IterableStateMachine::LoopContinue {
                        a,
                        k,
                        mut iterator_record,
                    } => {
                        iterator_record.update_result(result.clone()?, context)?;
                        ...
                        if let Some(mapfn) = &global_state.mapfn {
                            ...
                            state_machine.set(Some(IterableStateMachine::LoopEnd {
                                a,
                                k,
                                iterator_record,
                                mapped_value: None,
                            }));
                            return Ok(ContinuationState::Yielded(mapped_value));
                        }
                        sm = IterableStateMachine::LoopEnd {
                            a,
                            k,
                            iterator_record,
                    ...
                    IterableStateMachine::LoopEnd {
                        a,
                        k,
                        iterator_record,
                        mapped_value,
                    } => {
                        let mapped_value = match mapped_value.unwrap_or(result.clone()) {
                            Ok(value) => value,
                            Err(err) => {
                                sm = IterableStateMachine::AsyncIteratorCloseStart {
                                    err,
                        ...
                        sm = if let Err(err) =
                            a.create_data_property_or_throw(k, mapped_value, context)
                        {
                            IterableStateMachine::AsyncIteratorCloseStart {
                                err,
                                iterator: iterator_record.iterator().clone(),
                            }
                        } else {
                    ...
                    IterableStateMachine::AsyncIteratorCloseStart { err, iterator } => {
                        let Ok(Some(ret)) = iterator.get_method(js_string!("return"), context)
                        else {
                            return Err(err);
                        };
                        ...
                        state_machine
                            .set(Some(IterableStateMachine::AsyncIteratorCloseEnd { err }));
                        return Ok(ContinuationState::Yielded(value));
                    }
                    IterableStateMachine::AsyncIteratorCloseEnd { err } => {
                        return Err(err);
                    }
                }
            }
        ...
        fn from_array_like(
            result: JsResult<JsValue>,
            global_state: &GlobalState,
            state_machine: &Cell<Option<ArrayLikeStateMachine>>,
            context: &mut Context,
        ) -> JsResult<ContinuationState> {
            let Some(mut sm) = state_machine.take() else {
                return Ok(ContinuationState::Done);
            ...
            loop {
                match sm {
                    ArrayLikeStateMachine::LoopStart {
                        ...
                        return Ok(ContinuationState::Yielded(k_value));
                    }
                    ArrayLikeStateMachine::LoopContinue {
                        ...
                        let k_value = result.clone()?;
                        if let Some(mapfn) = &global_state.mapfn {
                            let mapped_value = mapfn.call(
                                &global_state.this_arg,
                                &[k_value, k.into()],
                                context,
                            )?;
                    ...
                    ArrayLikeStateMachine::LoopEnd {
                        ...
                        let mapped_value = mapped_value.unwrap_or(result.clone()?);
                        a.create_data_property_or_throw(k, mapped_value, context)?;
        ...
        let result: JsResult<()> = (|| {
                ...
                async_items.get_method(JsSymbol::async_iterator(), context)?
            {
                ...
                let continuation = NativeContinuation::from_copy_closure_with_captures(
                    |result, (gs, sm), context| match from_array_like(result, gs, sm, context) {
                        Ok(cont) => cont,
                        Err(err) => {
                            gs.resolvers
                                .reject
                ...
                if let ContinuationState::Yielded(value) =
                    continuation.call(Ok(JsValue::undefined()), context)
                {
                    JsPromise::resolve(value, context).await_native(continuation, context);
                }
            ...
            let continuation = NativeContinuation::from_copy_closure_with_captures(
                |result, (gs, sm), context| match from_iterable(result, gs, sm, context) {
                    Ok(cont) => cont,
                    Err(err) => {
                        gs.resolvers
                            .reject
                ...
                (
                    ...
                    Cell::new(Some(IterableStateMachine::LoopStart {
                        a,
                        k: 0,
                        iterator_record,
                    })),
