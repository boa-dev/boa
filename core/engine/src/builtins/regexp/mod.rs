...
impl RegExp {
    ...
    pub(crate) fn abstract_builtin_exec(
        ...
        let match_value = loop {
            ...
            let r: Option<regress::Match> = if full_unicode {
                matcher
                    .find_from_utf16(&input.to_vec(), last_index as usize)
                    .next()
            } else {
                matcher
                    .find_from_ucs2(&input.to_vec(), last_index as usize)
                    .next()
            };
        ...
        let matched_substr = js_string!(&input.to_vec()[(last_index as usize)..(e)]);
        ...
        let (groups, group_names) = if !named_groups.clone().is_empty() {
            ...
            for (name, range) in named_groups {
                ...
                if let Some(range) = range {
                    let value = js_string!(&input.to_vec()[range.clone()]);
        ...
        for i in 1..=n {
            ...
            let captured_value = capture.clone().map_or_else(JsValue::undefined, |range| {
                js_string!(&input.to_vec()[range]).into()
            });
    ...
    pub(crate) fn to_string(
        ...
        // 6. Return result.
        Ok(js_string!(js_str!("/"), &pattern, utf16!("/"), &flags).into())
    }
