    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(shadow_stack) = &self.backtrace {
            for entry in shadow_stack.iter().rev() {
                write!(f, "\n    at ")?;
                match entry {
                    ShadowEntry::Native { function_name, loc } => {
                        if let Some(loc) = loc {
                            write!(
                                f,
                                "{} (native at {}:{}:{})",
                    ...
                    ShadowEntry::Bytecode { pc, source_map } => {
                        let has_function_name = !source_map.function_name().is_empty();
                        if has_function_name {
                            write!(f, "{}", source_map.function_name().to_std_string_escaped(),)?;
                        } else {
                            f.write_str("<anonymous>")?;
                        }

                        f.write_str(" (")?;
                        source_map.source_path().fmt(f)?;

                        if let Some(position) = source_map.find(*pc) {
                            write!(
                                f,
                                ":{}:{}",
