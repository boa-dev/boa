...
trait TraceableContinuation: Trace {
    fn call(&self, value: JsResult<JsValue>, context: &mut Context) -> ContinuationState;
}

#[derive(Trace, Finalize)]
struct Continuation<F, T>
where
    F: Fn(JsResult<JsValue>, &T, &mut Context) -> ContinuationState,
    T: Trace,
{
    // SAFETY: `NativeContinuation`'s safe API ensures only `Copy` closures are stored; its unsafe API,
...
where
    ...
    T: Trace,
{
    fn call(&self, result: JsResult<JsValue>, context: &mut Context) -> ContinuationState {
...
impl NativeContinuation {
    ...
    where
        F: Fn(JsResult<JsValue>, &T, &mut Context) -> ContinuationState + Copy + 'static,
        T: Trace + 'static,
    {
    ...
    where
        F: Fn(JsResult<JsValue>, &T, &mut Context) -> ContinuationState + 'static,
        T: Trace + 'static,
    {
