...
trait TraceableCoroutine: Trace {
    fn call(&self, value: JsResult<JsValue>, context: &Context) -> JsResult<CoroutineState>;
}

#[derive(Trace, Finalize)]
struct Coroutine<F, T>
where
    F: Fn(JsResult<JsValue>, &T, &Context) -> JsResult<CoroutineState>,
    T: Trace,
{
...
where
    ...
    T: Trace,
{
    fn call(&self, result: JsResult<JsValue>, context: &Context) -> JsResult<CoroutineState> {
...
impl NativeCoroutine {
    ...
    where
        F: Fn(JsResult<JsValue>, &T, &Context) -> JsResult<CoroutineState> + Copy + 'static,
        T: Trace + 'static,
    {
    ...
    where
        F: Fn(JsResult<JsValue>, &T, &Context) -> JsResult<CoroutineState> + 'static,
        T: Trace + 'static,
    {
