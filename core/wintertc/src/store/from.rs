...
fn clone_data_view(
    ...
    let buffer = dataview.buffer(context)?;
    let buffer = try_from_js_value(&buffer, transfer, seen, context)?;
...
fn try_from_js_object_clone(
    ...
    } else if let Ok(ref dataview) = JsDataView::from_object(object.clone()) {
        return clone_data_view(object, dataview, transfer, seen, context);
