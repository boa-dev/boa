...
function activate(context) {
  ...
  try {
    ...
    const factoryDisposable =
      vscode.debug.registerDebugAdapterDescriptorFactory("boa", factory);
    ...
    const providerDisposable = vscode.debug.registerDebugConfigurationProvider(
      "boa",
      provider,
    );
    ...
    vscode.window.showInformationMessage(
      "Boa Debugger: Extension activated! Ready to debug.",
    );
  } catch (error) {
    console.error("[BOA EXTENSION] ❌ Activation failed:", error);
    vscode.window.showErrorMessage(
      `Boa Debugger activation failed: ${error.message}`,
    );
    throw error;
  }
}
...
class BoaDebugAdapterDescriptorFactory {
  ...
  async createDebugAdapterDescriptor(session) {
    console.log(
      `[Boa Debug] Creating debug adapter for session: ${session.name}`,
    );
    ...
    if (useHttp) {
      ...
      if (!portAvailable) {
        ...
        console.log(
          `[Boa Debug] Port ${httpPort} is already in use, connecting to existing server`,
        );
      } else {
        console.log(
          `[Boa Debug] Port ${httpPort} is available, starting new server`,
        );
        ...
        if (!boaCliPath) {
          const errorMsg =
            "boa-cli not found. Please ensure it is built in target/debug or target/release.";
        ...
        serverProcess.on("error", (err) => {
          ...
          vscode.window.showErrorMessage(
            `Failed to start Boa debug server: ${err.message}`,
          );
        });

        // Wait for the server to be ready
        console.log(
          `[Boa Debug] Waiting for server to be ready on port ${httpPort}...`,
        );
      ...
      console.log(
        `[Boa Debug] HTTP debug adapter descriptor created for port ${httpPort}`,
      );
      return descriptor;
    }
  ...
  findBoaCli() {
    ...
    if (workspaceFolders && workspaceFolders.length > 0) {
      ...
      if (boaRepoRoot) {
        ...
        console.log(
          `[Boa Debug] Could not find Boa repository root from: ${workspaceRoot}`,
        );
      }
    }
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (fs.existsSync(cargoTomlPath) && fs.existsSync(cliDirPath)) {
        ...
        try {
          ...
          if (
            cargoContent.includes("boa_cli") ||
            cargoContent.includes("boa_engine")
          ) {
            console.log(
              `[Boa Debug] ✓ Found Boa repository root at: ${currentPath}`,
            );
            return currentPath;
          }
        } catch (e) {
  ...
  resolveDebugConfiguration(folder, config, token) {
    ...
    if (!config.type && !config.request && !config.name) {
      ...
      if (editor && editor.document.languageId === "javascript") {
        ...
        console.log(
          `[Boa Debug] Created default config for: ${config.program}`,
        );
      }
    }
    ...
    if (!config.program) {
      const errorMsg =
        "Cannot debug: No program specified in launch configuration.";
      console.error(`[Boa Debug] ${errorMsg}`);
      vscode.window.showErrorMessage(errorMsg);
      return null;
    }
