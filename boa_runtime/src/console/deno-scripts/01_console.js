...
globalThis.__boa_no_color = () => false;

function getNoColor() {
  return globalThis.__boa_no_color() ?? (() => false);
}
...
function isAsyncFunction(value) {
  return (
    typeof value === "function" && value[SymbolToStringTag] === "AsyncFunction"
  );
}
...
function isMapIterator(value) {
  return isObjectLike(value) && value[SymbolToStringTag] === "Map Iterator";
}

function isModuleNamespaceObject(value) {
  return isObjectLike(value) && value[SymbolToStringTag] === "Module";
}
...
function isPromise(value) {
  return isObjectLike(value) && value[SymbolToStringTag] === "Promise";
}
...
function isSetIterator(value) {
  return isObjectLike(value) && value[SymbolToStringTag] === "Set Iterator";
}
...
const builtInObjects = new SafeSet(
  ArrayPrototypeFilter(ObjectGetOwnPropertyNames(globalThis), (e) =>
    RegExpPrototypeTest(builtInObjectsRegExp, e),
  ),
);
...
function reduceToSingleString(
  ...
  if (isBelowBreakLength(ctx, output, 0, base)) {
    return (
      `${braces[0]}${base ? ` ${base}` : ""} ${ArrayPrototypeJoin(
        output,
        ", ",
      )} ` + braces[1]
    );
...
function inspectArgs(args, inspectOptions = {}) {
  ...
  if (ctx.indentLevel > 0) {
    ...
    string =
      groupIndent + StringPrototypeReplaceAll(string, "\n", `\n${groupIndent}`);
  }
...
globalThis.console = new Console(globalThis.__boa_print);
