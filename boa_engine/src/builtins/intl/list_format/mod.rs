...
impl BuiltInConstructor for ListFormat {
    ...
    fn constructor(
        ...
        let typ = get_option(&options, "type", context)?.unwrap_or_default();

        // 13. Let style be ? GetOption(options, "style", string, « "long", "short", "narrow" », "long").
        // 14. Set listFormat.[[Style]] to style.
        let style = get_option(&options, "style", context)?.unwrap_or(ListLength::Wide);
...
impl ListFormat {
    ...
    fn format_to_parts(
        ...
        for (n, part) in parts.0.into_iter().enumerate() {
            ...
            o.create_data_property_or_throw("type", js_string!(part.typ()), context)
                .expect("operation must not fail per the spec");

            // c. Perform ! CreateDataPropertyOrThrow(O, "value", part.[[Value]]).
            o.create_data_property_or_throw("value", js_string!(part.value()), context)
                .expect("operation must not fail per the spec");
    ...
    fn resolved_options(
        ...
        options
            .create_data_property_or_throw("locale", js_string!(lf.locale.to_string()), context)
            .expect("operation must not fail per the spec");
        options
