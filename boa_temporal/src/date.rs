...
impl TemporalDate {
    ...
    pub fn move_relative_date(&self, duration: &Duration) -> TemporalResult<(TemporalDate, f64)> {
        let new_date = self.add_to_date(duration, ArithmeticOverflow::Constrain)?;
    ...
    /// `DifferenceDate`
    pub fn diff_date(&self, other: &Self, largest_unit: TemporalUnit) -> TemporalResult<Duration> {
