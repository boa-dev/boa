...
//!
//! Some environments are stored as `JSObjects`. This is for GC, i.e we want to
//! keep an environment if a variable is closed-over (a closure is returned).
//! All of the logic to handle scope/environment records are stored in here.
//!
//! There are 5 Environment record kinds. They all have methods in common, these
//! are implemented as a the `EnvironmentRecordTrait`
//!
use crate::{
...
/// <https://tc39.es/ecma262/#sec-environment-records>
///
/// In the ECMAScript specification Environment Records are hierachical and have
/// a base class with abstract methods. In this implementation we have a trait
/// which represents the behaviour of all `EnvironmentRecord` types.
pub trait EnvironmentRecordTrait: Debug + Trace + Finalize {
    ...
    /// argument deletion is true the binding may be subsequently deleted.
    ...
    /// If strict is true then attempts to set it after it has been initialized
    /// will always throw an exception, regardless of the strict mode
    ...
    /// Record. The String value `name` is the text of the bound name.
    /// value is the `value` for the binding and may be a value of any
    /// ECMAScript language type. S is a Boolean flag. If `strict` is true
    ...
    /// If the binding exists but cannot be removed return false. If the binding
