...
impl<R> TokenParser<R> for AsyncGeneratorExpression where
    R: Read,
    {
        ...
        fn parse(self, cursor: &mut Cursor<R>) -> Result<Self::Output, ParseError> {
            ...
            let params_start_position = cursor.expect(Punctuator::OpenParen, "async generator expression")?.span().end();
