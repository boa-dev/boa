...
pub(super) fn call_construct(
    ...
    ///
    /// This is needed for the call method since we cannot mutate the function since we
    ...
    let body = if let Some(function) = obj.borrow_mut().as_mut_function() {
            ...
            match function {
                Function::Native {
                    ...
                    if !*constructor || construct {
                        ...
                        return function(&JsValue::Undefined, args, context);
                    }
                }
                Function::Closure {
                    function, captures, ..
