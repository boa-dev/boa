    ...
    fn execute_instruction(&mut self) -> JsResult<bool> {
        ...
        match opcode {
            ...
            Opcode::GetPropertyByName => {
                ...
                let name = self.vm.frame().code.variables[index as usize];
                let key = PropertyKey::from(self.interner().resolve_expect(name));
            ...
            Opcode::SetPropertyGetterByName => {
                ...
                let name_sym = self.vm.frame().code.variables[index as usize];
                let name = self.interner().resolve_expect(name_sym).into();
                let set = object
            ...
            Opcode::DeletePropertyByName => {
                let index = self.vm.read::<u32>();
                let key_sym = self.vm.frame().code.variables[index as usize];
                let key = self.interner().resolve_expect(key_sym).into();
                let object = self.vm.pop();
