...
use crate::{
    ...
    expression::{
        ...
        OptionalOperationKind, Parenthesized, RegExp, Spread, SuperCall, TaggedTemplate, Yield,
    },
    function::{
...
pub trait Visitor<'ast>: Sized {
    ...
    define_visit!(visit_reg_exp, RegExp);
    ...
    fn visit<N: Into<NodeRef<'ast>>>(&mut self, node: N) -> ControlFlow<Self::BreakTy> {
        ...
        match node {
            ...
            NodeRef::RegExp(n) => self.visit_reg_exp(n),
...
pub trait VisitorMut<'ast>: Sized {
    ...
    define_visit_mut!(visit_reg_exp_mut, RegExp);
    ...
    fn visit<N: Into<NodeRefMut<'ast>>>(&mut self, node: N) -> ControlFlow<Self::BreakTy> {
        ...
        match node {
            ...
            NodeRefMut::RegExp(n) => self.visit_reg_exp_mut(n),
